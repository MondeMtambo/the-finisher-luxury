...
def send_onboarding_credentials_email(user, temp_password):
    subject = "Welcome to THE FINISHER LUXURY - Temporary Login Password"
    display_name = user.first_name or user.username
    message = (
        f"Hi {display_name},\n\n"
        "Welcome to THE FINISHER LUXURY.\n"
        "Please use this temporary password to log in and verify your account:\n\n"
        f"Temporary Password: {temp_password}\n\n"
        "After your first login, you'll receive a 6-digit verification code by email.\n"
        "Please change your password immediately after accessing your dashboard.\n\n"
        "- MTAMBO HOLDINGS"
    )
    send_mail(
        subject=subject,
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        generated_temp_password = generate_temp_password()
        ...
        try:
            with transaction.atomic():
                ...
                email_error = ''
                try:
                    send_onboarding_credentials_email(new_user, generated_temp_password)
                except Exception as ex:
                    email_status = 'failed'
                    email_error = str(ex)
                ...
                payload['onboarding_email_status'] = email_status
                payload['onboarding_email_message'] = (
                    'CEO Temp password sent to user email.'
                    if email_status == 'sent' else
                    'User was created, but temp password email failed to send. Please resend from admin.'
                )
                if email_error:
                    payload['onboarding_email_error'] = email_error
